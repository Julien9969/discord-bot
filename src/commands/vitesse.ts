...
export async function execute(interaction: CommandInteraction) {
    ...
    const resource = createAudioResource("./assets/Initial D.mp3", {
    ...
    //     console.log("destroy");
    //     const resource = createAudioResource("./assets/ring.mp3", {inputType: StreamType.Arbitrary, inlineVolume: true});
    ...
    try {
      ...
      if (confirmation?.customId === "stop-button") {
        connection.destroy();
        interaction.editReply({ content: "Stop", components: [] });
      }
    } catch (e: unknown) {
